...
export class OptFrontend extends AbstractBaseFrontend implements IOptFrontend{
    ...
    constructor(params={}){
        ...
        $('#genUrlBtn').bind('click', () => {
            let myArgs = this.getAppState();
            let urlStr = $.param.fragment(window.location.href, myArgs, 2); // 2 means 'override'
        ...
        /*  periodically do submitUpdateHistory() to handle the case when
            ...
            re-editing code; that way, we can still get some signals rather than nothing. */
        let lastSubmittedUpdateHistoryLength = 0;
        setInterval(() => {
            if (this.myVisualizer) {
                let uh = this.myVisualizer.updateHistory;
    ...
    submitUpdateHistory(why: any): void {
        if (this.myVisualizer) {
            let encodedUh = this.compressUpdateHistoryList();
            let encodedUhJSON = JSON.stringify(encodedUh);
    ...
    initAceEditor(height: number): void {
        ...
        let s = this.pyInputAceEditor.getSession();
        ...
        $('#codeInputPane').css('height', height + 'px'); // VERY IMPORTANT so that it works on I.E.
    ...
    setAceMode(): void {
        let selectorVal = $('#pythonVersionSelector').val();
        let mod;
        let tabSize = 2;
        let editorVal = $.trim(this.pyInputGetValue());
    ...
    pyInputSetValue(dat: string): void {
        this.pyInputAceEditor.setValue(dat.trim() /* kill trailing spaces */, -1 /* do NOT select after setting text */);
        $('#urlOutput,#urlOutputShortened,#embedCodeOutput').val('');
        this.clearFrontendError();
        /*  scroll to top to make the UI more usable on smaller monitors
    ...
    pyInputSetScrollTop(st: any): void {
    ...
    executeCodeFromScratch(): void {
        ...
        if (this.pyInputAceEditor && $.trim(this.pyInputGetValue()) == '') {
            this.setFrontendError(["Type in some code to visualize."]);
            return;
        }
        super.executeCodeFromScratch();
    }

    executeCode(forceStartingInstr = 0, forceRawInputLst = undefined): void {
        /*  if you're in display mode, kick back into edit mode before executing
            or else the display might not refresh properly */
    ...
    finishSuccessfulExecution(): void {
        ...
        if (this.myVisualizer) {
            this.myVisualizer.add_pytutor_hook("end_updateOutput", (args) => {
                ...
                if (args.myViz.creationTime) {
                    ...
                    let uh = args.myViz.updateHistory;
        ...
        /*  each element will be a two-element list consisting of:
            ...
            the invariant is that step number < total # steps (since it's zero-indexed) */
        ...
        if (trace.length == 1 && trace[0].line) {
            ...
            if (errorLineNo !== undefined && errorLineNo != NaN) {
                ...
                /*  if we have both a line and column number, then move over to
                    that column. (going to the line first prevents weird highlighting bugs) */
    ...
    snapshotCodeDiff(): void {
        ...
        let newCode = this.pyInputGetValue();
        let timestamp = new Date().getTime();

        if (this.curCode != newCode) {
            let diff = this.dmp.diff_toDelta(this.dmp.diff_main(this.curCode, newCode));
            let delta = { t: timestamp, d: diff };
    ...
    updateAppDisplay(newAppMode: any): void {
        ...
        if (this.appMode === undefined || this.appMode == 'edit' || !this.myVisualizer) {
            this.appMode = 'edit'; // canonicalize -- if no visualizer, default to edit mode
            ...
            let v = $('#pythonVersionSelector').val();
            if (v === 'js' || v === '2' || v === '3') {
                let myArgs = this.getAppState();
                let urlStr = $.param.fragment('live.html', myArgs, 2 /* clobber all */);
    ...
    parseQueryString(): void {
        let queryStrOptions = this.getQueryStringOptions();
        ...
        if (queryStrOptions.appMode == 'display' && queryStrOptions.preseededCode /* jump to 'display' mode only with preseeded code */) {
