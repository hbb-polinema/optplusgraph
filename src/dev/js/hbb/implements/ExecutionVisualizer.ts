...
export class ExecutionVisualizer implements IExecutionVisualizer {
    ...
    /*  represent the current state of the visualizer object;
        i.e. which step is it currently visualizing? */
    ...
    /*  true if trace ended prematurely since
        maximum instruction limit has been reached */
    ...
    /*  Constructor with an ever-growing feature-crepped list of options :)
        ...
        updateOutputCallback - function to call (with 'this' as parameter)
                            whenever this.updateOutput() is called (BEFORE rendering the output display)
        heightChangeCallback - function to call (with 'this' as parameter) whenever the HEIGHT of #dataViz changes
        verticalStack - if true, then stack code display ON TOP of visualization (else place side-by-side)
    ...
    constructor(domRootID, dat, params) {
        this.curInputCode = dat.code.trim(); // kill trailing spaces
        this.params = params;
        ...
        if (this.curTrace.length > 0) {
            ...
            /*  if the final entry is raw_input or mouse_input, then trim it from the trace and
                set a flag to prompt for user input when execution advances to the end of the trace*/
        ...
        let tmpRoot = $('#' + domRootID);
        let tmpRootD3 = d3.select('#' + domRootID);
        ...
        if (this.params.lang === 'java') {
            this.activateJavaFrontend(); // do this before initializing codeOutputLines (order dependency)
        }

        let lines = this.curInputCode.split('\n');
        for (let i = 0; i < lines.length; i++) {
            let cod = lines[i];
            let n: { text: string, lineNumber: number, executionPoints: number[], breakpointHere: boolean } = {
                text: cod,
                lineNumber: i + 1,
                executionPoints: [],
            ...
            let breakpointInComment = false;
            let toks = cod.split('#');
            for (let j = 1 /* start at index 1, not 0 */; j < toks.length; j++) {
    ...
    add_pytutor_hook(hook_name: any, func: any): void {
    ...
    try_hook(hook_name: any, args: any): [boolean] {
        if (this.pytutor_hooks[hook_name]) {
            for (let i = 0; i < this.pytutor_hooks[hook_name].length; i++) {
                // apply w/o "this", and pack sole arg into array as required by apply
                let handled_and_result
    ...
    generateID(original_id: any): string {
    ...
    render() {
        ...
        if (this.params.startingInstruction) {
            ...
            // weird special case for something like: e=raw_input(raw_input("Enter something:"))
            ...
            Assert(0 <= this.params.startingInstruction && this.params.startingInstruction < this.curTrace.length);
            this.curInstr = this.params.startingInstruction;
        }

        if (this.params.jumpToEnd) {
            let firstErrorStep = -1;
            for (let i = 0; i < this.curTrace.length; i++) {
                let e = this.curTrace[i];
            ...
            // set to first error step if relevant since that's more informative than simply jumping to the very end
            if (firstErrorStep >= 0) {
    ...
    _getSortedBreakpointsList() {
        ...
        ret.sort(function (x, y) { return x - y }); // JavaScript sort is lexicographic by default!
        return ret;
    }

    addToBreakpoints(executionPoints: any): void {
    ...
    removeFromBreakpoints(executionPoints: any): void {
        $.each(executionPoints, (i, ep) => {
    ...
    findPrevBreakpoint() {
        ...
        if (this.sortedBreakpointsList.length == 0) {
            ...
            for (let i = 1; i < this.sortedBreakpointsList.length; i++) {
                let prev = this.sortedBreakpointsList[i - 1];
                let cur = this.sortedBreakpointsList[i];
                if (c <= prev)
                    return -1;
                if (cur >= c)
                    return prev;
            ...
            // final edge case:
            let lastElt = this.sortedBreakpointsList[this.sortedBreakpointsList.length - 1];
    ...
    findNextBreakpoint() {
        ...
        /*  usability hack: if you're currently on a breakpoint, then
            ...
            for (let i = 0; i < this.sortedBreakpointsList.length - 1; i++) {
                let cur = this.sortedBreakpointsList[i];
                let next = this.sortedBreakpointsList[i + 1];
                if (c < cur)
                    return cur;
    ...
    stepForward(): boolean {
        ...
        if (myViz.curInstr < myViz.curTrace.length - 1) {
            ...
            if (myViz.sortedBreakpointsList.length > 0) {
                let nextBreakpoint = myViz.findNextBreakpoint();
                if (nextBreakpoint != -1)
            ...
            return true; // returns true if action successfully taken
        }
    ...
    stepBack(): boolean {
        ...
        if (myViz.curInstr > 0) {
            ...
            if (myViz.sortedBreakpointsList.length > 0) {
                let prevBreakpoint = myViz.findPrevBreakpoint();
                if (prevBreakpoint != -1)
    ...
    updateOutput(smoothTransition = false): void {
        if (this.params.hideCode) {
    ...
    updateOutputFull(smoothTransition: any): void {
        ...
        let prevDataVizHeight = myViz.dataViz.height();
        ...
        if (isLastInstr && myViz.params.executeCodeWithRawInputFunc && myViz.promptForUserInput) {
    ...
    activateJavaFrontend(): void {
        ...
        this.curTrace.forEach((e, i) => {
            /*  the Java backend doesn't attach line numbers to exception
        ...
        this.add_pytutor_hook(
            ...
            function (args) {
                let obj = args.obj, d3DomElement = args.d3DomElement;
                let typ = typeof obj;
                if (obj instanceof Array && obj[0] == "VOID") {
                    ...
                    let asc = obj[1].charCodeAt(0);
                    let ch = obj[1];

                    // default
                    let show = asc.toString(16);
        ...
        this.add_pytutor_hook(
            ...
            function (args) {
                ...
                if (myViz.params.stdin && myViz.params.stdin != "") {
                    let stdinHTML = '<div id="stdinWrap">stdin:<pre id="stdinShow" style="border:1px solid gray"></pre></div>';
        ...
        this.add_pytutor_hook(
            ...
            function (args) {
                ...
                let d3DomElement = args.d3DomElement;
                let obj = args.obj;
                let typeLabelPrefix = args.typeLabelPrefix;
                let myViz = args.myViz;
                let stepNum = args.stepNum;

                if (!(obj[0] == 'LIST' || obj[0] == 'QUEUE' || obj[0] == 'STACK'))
                    return [false]; // didn't handle

                let label = obj[0].toLowerCase();
                let visibleLabel = { list: 'array', queue: 'queue', stack: 'stack' }[label];
                ...
                if (obj[0] == 'LIST') {
                    ...
                    for (let i = 1, ind = 0; i < obj.length; i++) {
                        let val = obj[i];
                        let elide = val instanceof Array && val[0] == 'ELIDE';
        ...
        let old_generateID = ExecutionVisualizer.prototype.generateID.bind(this);
        this.generateID = function (original_id) {
            let sanitized = original_id.replace(
                /[^0-9a-zA-Z_]/g,
    ...
    updateLineAndExceptionInfo(): void {
        ...
        let isLastInstr = myViz.curInstr === (totalInstrs - 1);
